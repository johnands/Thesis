...
\begin{abstract}
 We implement a method for constructing analytic potentials by fitting artifical neural networks to 
 data sampled from molecular dynamics simulations. 
 The LAMMPS distribution is extended with custom classes for sampling training data and applying 
 neural network potentials in simulatons. Further, TensorFlow is employed 
 to train neural networks. The Lennard-Jones potential is reproduced, and various properties of the methodology disussed.  
 ...
 and the bulk modulus with Stillinger-Weber simulations and/or experiments. The crystal structure of Si is reproduced with 
 small differences, and the bulk modulus is measured to be similar to experimental values. 
\end{abstract}
...
employing the Behler-Parinello method for NN interpolation \cite{Behler07} \cite{Artrith12} \cite{Natarajan16} .  
