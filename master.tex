...
like noble gases well. It is a two-body potential that only depends on the distance $r$ between pairs of atoms. 
...
\begin{equation}
 V_{tot} = \sum_i\sum_{j>i} V_2(r_{ij}) = \frac{1}{2}\sum_i\sum_j V_2(r_{ij})
...
from $i$ to $k$, illustrated in \autoref{fig:triplets}. 
\autoref{fig:triplets} illustrates 
\begin{equation}
 V_{tot} = \sum_i\sum_{j\neq i}\sum_{k>j} V_3(r_{ij}, r_{ik})
 \label{threeBodyPotentialEnergy}
\end{equation}
\begin{figure}
\begin{center}
...
and a three-body term. The general expression for a three-body potential written in inter-atomic coordinates is
\begin{equation}
 V_{3\mathrm{body}} = \sum_i\sum_{j>i}V_2(r_{ij}) + \sum_i\sum_{j\neq i}\sum_{k>j} V_3(r_{ij}, r_{ik}, \theta_{ijk})
 \label{threeBodyPotential}
\end{equation}
Comparing with \eqref{generalPotential}, the only difference is the sum indicies
