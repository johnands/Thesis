...
training set is close to zero \cite{LeCun12}.
...
Thus, both scaling and shifting of the training data can be performed to speed up the fitting process.  
...
\subsection{Hyperparameters} \label{sec:determineHyperparameters}
At the end of \autoref{sec:symmetryFunctions}, we briefly mentioned that a number of so-called hyperparameters must be set
prior to the actual weight optimization. These include, among others, the learning rate and 
the architecture of the NN, i.e. the number of layers and nodes. Choosing hyperparameter values can be treated as a
...
There is no standard and accepted method for finding the optimal size of a NN for a specific data set, but 
we do have some general ground rules.
In this work, the input layer is a set of symmetry functions that acts as descriptors of atomic configurations.
(SHOULD THE DISCUSSION ON THE INPUT LAYER BE MOVED TO THE SECTION ON CONSTRUCTIONS OF SYMMETRY FUNCTION SETS?)
The number of symmetry functions must be large enough for the NNP to be able to distinguish distinct atomic structures that
...
of the system under investigation with an empirical potential of choice, and measure coordination numbers. When the 
target potential is unknown, the information can be extracted from empirical knowledge of the system. 
...
there are more atomic triplets than pairs for a given ensemble of atoms. One could immidiately conclude that we
...
considering that each function describes the \textit{whole} cutoff sphere. 
...
Many nodes means many parameters that can be tuned to the training set, resulting in a better fit.
However, if the number of neurons is too large, the network will \textit{overfit} the training data. 
In this case, the network fits the training set very well, but interpolation between the training points is poor. Thus, 
the NNP becomes less predictive, and will not perform well in simulations. This problem is addressed below \autoref{sec:overfitting}.  
...
Further, in the literature we have studied concerning NNPs, such as \cite{Behler07} \cite{Raff05} \cite{Witkoskie05}, 
only one or two hidden layers are used. 



\subsubsection{Activation function and saturation}

\subsubsection{Symmetry functions}

\subsubsection{Cost function}

\subsubsection{Learning rate and mini-batch size}

\subsubsection{Number of iterations and overfitting} \label{sec:overfitting}
