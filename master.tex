...
fit the training data at hand. We follow and extend upon the discussions in \cite{Behler11symmetry} and \cite{Behler15}. 
...
adequately probe the configuration space defined by the training data. There are two requirements
...
A common method to detect overfitting is \textit{early stopping}. Here, the reference data is randomly split into 
a training set, which is employed for the optimization of the weights, and a test set, which 
...
\subsection{Activation function and saturation}
...
\subsection{Number of iterations and overfitting} \label{sec:overfitting}
