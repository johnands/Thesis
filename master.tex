...
\subsection{From quantum mechanics to classical potentials}
...
\subsection{Constructing potential energy surfaces}
...
\begin{itemize}
 ...
 \item Quantum mechanical potentials: A computational quantum mechanics (QM) method 
 is used to obtain the potential energy for different atomic configurations. The configurations and corresponding energies
 are then fitted by a functional form. 
 \item Semi-empirical potentials: Mainly based on methods from computational quantum mechanics (QM), but many approximations
 are made and some parameters are obtained from empirical data.
\end{itemize}
In this thesis we will present a method to construct quantum mechanial potentials, where atomic configurations and energies are fitted
by artifical neural networks. 

In the following section, we describe how the functional form of a PES can be constructed.  
...
Empirical potentials are often quite simple and only have a few parameters, making their performance superior. However, their
...
be viewed as \textit{ab inito} potentials. 
Potentials of this type can be very accurate due to their unbiased nature, and the method can in principle be applied to 
any system. 


Determining how many terms that is sufficient to 
...
silicon (Si), will need more terms. Another challenge is to identify the
configuration space of the system. For three-body potentials and more the set of probable configurations is large, and 
running electronic structure calculations for each configuration is expensive.

In this thesis we present a method to construct potential energy surfaces 
