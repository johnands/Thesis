...
\begin{equation}
 u_i^1 = \sum_{m=1}^2 x_m w_{m,i}^1 + b_i^1 
\end{equation}
...
\begin{equation}
 y_i^1 = f^1(u_i^1) = f^1\left(\sum_{m=1}^2 x_m w_{m,i}^1 + b_i^1\right)
 \label{outputLayer1}
\end{equation}
...
\begin{equation}
 y_i^l = f^l(u_i^l) = f^l\left(\sum_{m=1}^{N_{l-1}} y_m^{l-1}w_{m,i}^l + b_i^l\right)
 \label{generalLayer}
\end{equation}
...
\begin{align}
 y_i^2 &= f^2\left(\sum_{m=1}^3 y_m^1w_{m,i}^2 + b_i^2\right) \\
 &= f^2\left[\sum_{m=1}^3 f^1\left(\sum_{k=1}^2 x_n w_{k,m}^1 + b_m^1\right)w_{m,i}^2 + b_i^2\right]
\end{align}
where we have substituted $y_m^1$ with \eqref{outputLayer1}. Finally, the NN output yields,
\begin{align}
 y_1^3 &= f^3\left(\sum_{m=1}^3 y_m^2w_{m,1}^3 + b_1^3\right) \\
 &= f^3\left[\sum_{m=1}^3 f^2\left(\sum_{k=1}^3 f^1\left(\sum_{j=1}^2 x_j w_{j,k}^1 + b_k^1\right)w_{k,m}^2 + b_m^2\right)
 w_{m,1}^3 + b_1^3\right]
\end{align}
We can generalize this expression to a MLP with $l$ hidden layers. The complete functional form
is,
\begin{equation}
 y^{l+1}_1 = f^{l+1}\left[\sum_{m=1}^{N_l} f^l\left(\sum_{k=1}^{N_{l-1}}f^{l-1}\left(
 \cdots f^1\left(\sum_{j=1}^{N_0} x_j w_{j,k}^1 + b_k^1\right)
 \cdots \right) w_{k,m}^2 + b_m^2\right)
 w_{m,1}^3 + b_1^3\right]
 \label{completeNN}
\end{equation}
which illustrates a basic property of MLPs: the only \textit{variables} are the input values $x_j$. 
...
In our example (\autoref{fig:networkNotation}) $n=2$ and $m=1$.  
...
two adjacent layers can be represented as a matrix $W$, where $w^l_{ij}$ is as before the weight connecting connecting node $j$
in layer $l-1$ with node $i$ in layer $l$. Additionaly, we can represent the biases and activations
as layer-wise column vectors, so that e.g. $\vec{b}^l_i$ is the bias of node $i$ in layer $l$. 
We have that $W^l$ is a $N_{l-1} \times N_l$ matrix, while $\vec{b}^l$ and $\vec{y}^l$ are $N_l \times 1$ column vectors. 
...
\begin{equation}
 y^2_i = w^2_{i1}y^1_1 + w^2_{i2}y^1_2 + w^2_{i3}y^1_3
\end{equation} 
...
atomic coordinates to the corresponding energies. The input values to the function that we want to fit are
are called the \textit{training set}, while the output values are called the \textit{reference set} or the \textit{target values}.
Interpolation with NNs is essentialy an optimization problem:
...
\begin{equation}
 ...
 \label{generalCost}
\end{equation}
where $W$ and $B$ are the NN's weights and biases respectively, $X$ is the training data and $Y$ is the target values. 
...
The most common cost function used in regression with NNs is the mean-square error,
\begin{equation}
 \Gamma = \frac{1}{2N}\sum_{i=1}^N (Y_i - y_i)^2
 \label{quadraticCost}
\end{equation}
where $y_i = y_i(W, B, X_i)$ is the value predicted by the NN for traing data sample $X_i$.  
The presence of the constant $1/2$ is explained in \autoref{sec:backprop}.  

Other cost functions are used for other ML tasks. 
