...
GENERELT OM ANNS I FORSKNING
The field of artifical intelligence (AI) research has experienced a surge of interest in 
the new millenium, mainly due to the recent success of artifical neural networks (ANNs). 
ANNs are computer programs that try to mimic biological brains
...
\cite{Helmstaedter13}. Further, physicists have developed ANNs that can solve the quantum many-body problem
...
OM ANNS I MD
The main strength of ANNs is their ability to learn features and find patterns in data sets. 
...
as the gradient of a pre-determined potential energy surface (PES), and integrate the atom trajectories according 
to Newton's equations of motion. The PES thus determines the dynamics of the system, and is the 
key factor for a MD simulation to be physically plausible.

MÅTER Å KONSTRUERE POTENSIALER PÅ
Atomic interactions are governed by the laws of quantum mechanics (QM). 
The most accurate approach to MD simulations is to calculate the PES on-the-fly with a QM method like 
...
In \textit{classical} MD, the PES is approximated by a \textit{pre-defined} mathematical function. 
The traditional strategy is to start out with a functional form 
...
a few parameters, making them fast to evaluate, but also biased and possibly inaccurate.  
Examples are the Lennard-Jones \cite{Jones24} and the Stillinger-Weber 
\cite{Stillinger85} potentials. 

A third option, which is used in this work, is to employ a QM method to produce a data set of atomic configurations and 
their corresponding energies. Then, a general functional form containing many parameters are fitted to this data set.
The resulting PES will be much faster to evaluate compared to computing energies and forces on-the-fly, but considerably slower than 
most empirical potentials. Further, if the fitting error is small, only the error of the underlying QM method is present. 
Thus, this scheme can be viewed as a multi-scale physics tool that bridges the gap between the quantum realm and the classical realm, 
enabling large-scale MD simulations with \textit{ab inito} accuracy. 

\section{Neural Network potentials}
To construct an adequate PES, we need a suitable fitting method to interpolate
atomic configurations and energies. The functional form that is to be fitted should be unbiased, contain the 
right amount of parameters

Several methods have been proposed, like interpolating moving least squares \cite{Dawes08} 
and modified Sheppard interpolation \cite{Ischtwan94}. In the past two decades, also ANNs have been employed for this purpose.
ANNs are purely mathematical functions that have a very flexible functional form. 


\section{Goals}
The main goal of this thesis is to use artificial neural networks to
construct many-body potential energy surfaces (PES) and apply them in molecular dynamics (MD) simulations.
