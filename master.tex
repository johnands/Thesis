...
\section{Force calculations and cutoff radius}
...
At the end of this chapter, we will discuss which of these requirements that are met by our NNPs. 
...
\cite{Behler07}. The two methods differ in the way each atomic NN is structured. 

In the first method, each atomic energy $E_i$ is computed by a NN of variable size, using a set of 
vectors of atomic chains as input, representing the environment of each atom. In the Behler-Parinello (BP) method, 
...
\begin{figure}
\begin{center}
  \includegraphics[width = 0.7\linewidth]{Figures/Theory/cutOffHighDimNN.jpeg}
  \caption{}
