...
The NNP simulations are also carried out for several temperatures $T \in [0,500] \mathrm{K}$. 

The iterative sampling process is stopped when no new structures are found, which occurs after three iterations 
of NNP sampling and consecutive fits. We do not employ the multiple-NN method by Behler (\autoref{sec:iterativeMDsampling}
to find new structures \textit{within} the range of existing symmetry values.
Instead, we plot the distribution of symmetry function values for the final data set to ensure that all intermediate 
values are covered. 



\part{Conclusions and future work}
In this thesis, we aimed to implement and employ a method to construct high-dimensonal neural network potentials for molecular 
dynamics. 

To train neural networkss, we developed a set of Python scripts that employ the machine learning 
library TensorFlow. These programs were used to fit data sets of atomic configurations and energies to construct potentials 
...
***Summary of what we have done and how good results we obtained***

***Discussion on how well the method is compared to the ideal potential***
