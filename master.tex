...
\begin{equation}
 \tau = 
\begin{cases}
  \textrm{floor}\left[\alpha/F_i\right], 
  &F_i \leq \alpha \\
  1, 
  &F_i > \alpha
\end{cases}
\end{equation}
where $\tau$ is measured in units of $\Delta t$ and $F_i$ is the total force on atom $i$ including ghost contributions. 
...
\end{minipage}
  \caption{Distribution of forces for one Si atom in a quartz ($\textrm{SiO}_2$) crystal with 
	   and withouth the use of the sampling algorithm \eqref{variableIntervalSampling}. 
	   The crystal consists of 576 atoms, with an initial temperature $T= \SI{1000}{\kelvin}$, 
	   run for $N = 10000$ time steps}.
\label{fig:forceDistSamplingAlgo}
\end{figure}
In \autoref{fig:forceDistSamplingAlgo}
