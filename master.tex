...
ADVANTAGES AND DISADVANTAGES OF NNPS - DIFFERENT METHODS TO CONSTRUCT NNPS IN THE LITERATURE - 
DESCRIPTION OF BEHLER-PARINELLO METHOD. 
...
fulfill important physical symmetries. We start out with a brief discussion on what properties that
are favourable for any PES, irregardles of circumstances. 
...
At the end of this chapter, we will discuss which of these requirements that are met by the NNPs constructed
in this thesis. 
...
All Si atoms have idential atomic NNs and symmetry function sets, the former represented by the function $N_{\mathrm{Si}}$. 
...
atom $i$ to all neighbours $j \in 1,\dots,n$, where $r_{ij} = |\vec{r}_j - \vec{r}_i|$. 
Further, $\vec{G}_\mathrm{Si}(\vec{R}_{ij}) = \{G_{\mathrm{Si},s}(\vec{R}_{ij})\}$ is the vector of Si symmetry functions
$s \in 1,\dots,M_\mathrm{Si}$. 
The process of calculating the potential energy $E_i$ described above can then be written,
\begin{equation}
 \vec{R}_{ij} \rightarrow \vec{G}_\mathrm{Si}(\vec{R}_{ij}) \rightarrow \mathrm{N}_\mathrm{Si}[\vec{G}_\mathrm{Si}(\vec{R}_{ij})] 
 \rightarrow E_i
...
While the symmetry function vector of an element $e$ is of a fixed size equal to the number of inputs to $N_e$,
...
Physically, \label{G1} can be thought of as a ''coordination number'' on $R_{ij} \in [0,R_c]$.
...
yielding coordination numbers on distance intervals of varying length. 
...
to use at least as many symmetry functions as there are degrees of freedom in the system.
...
\subsection{should this come at the end?}
NNPs offer a number of advantages for the construction of PESs \cite{Behler11general}. 
\begin{itemize}
 \item Energies can be fitted to arbitrary accuracy, leaving only
 the underlying error of the reference data. Consequentially, NNPs can be categorized as \textit{ab inito} potentials. 
 \item Evaluation of NNPs require much less CPU time than QM methods.
 \item The NNP expression is unbiased and generally applicable to any system.
\end{itemize}
In other words, the overarching vision is to create a PES that have the accuracy of \textit{ab inito} potentials, 
while simultaneously being able to simulate large systems over considerable time scales. 
Still, there are disadvantages to be aware of:
\begin{itemize}
 \item The evaluation of NNPs are notably slower compared to empirical potentials.
