...
\begin{abstract}
 We implement a method for constructing analytic interatomic potentials by fitting artificial neural networks to 
 ...
 and various mechanical properties with Stillinger-Weber simulations. 
...
In later years, NNs have also been employed in the natural sciences, where they have outperformed other machine learning
techniques at analysing particle accelerator data \cite{Ciodaro12} and reconstructing brain circuits
...
This has led to their application in the construction of interatomic potentials for molecular dynamics (MD) simulations, 
...
In \textit{classical} MD, the PES is instead approximated by a \textit{predefined} mathematical function. 
...
\begin{enumerate}[label=\textbf{\alph*)}]
 ...
 We also wish to write code that can analyse and visualize data sets and symmetry function sets. 
...
can be separated, known as the Born-Oppenheimer approximation. 
...
The basic theory of NNs is presented in chapter 2, while chapter 3 deals with physical symmetries that has to be considered
...
\centering
 ...
 \caption{The Lennard-Jones potential \eqref{Lennard-Jones} as a function of interatomic distance for two different parameter sets. 
 The depth of the potential well (potential
 strength) is given by $\epsilon$, while $\sigma$ is the interatomic distance for which the potential is zero.}
 \label{fig:LJ}
\end{figure}
...
two interatomic distances to the central atom $i$, and $\theta_{jik}$ is the angle between the lines from $i$ to $j$ and 
...
of elements; the parameter set must be fitted to e.g.\ (Si,Si,Si) and (Si,O,O) separately. The angle $\theta_{0jik}$ has 
...
The Vashishta potential has also been successfully employed on other multi-component systems like SiC \cite{Vashishta07}. 
...
is exactly zero at this point. The truncated and shifted form of the potential is
\begin{equation}
...
\begin{itemize}
 ...
 The goal is to construct a function that maps input data to continuous output values. 
...
of artificial neurons. However, we note that the above description is a highly simplified picture of the great complexity
...
An artificial neural network (NN), is a computational model that consists of layers of connected neurons, or \textit{nodes}. 
...
The main factor that separates the different types are how the neurons are \textit{connected}. 
...
a separate type of NN due the unusual activation functions (common activation functions are presented in
...
and an output layer only. It served as a \textit{binary classifier}, i.e.\ a function that maps a real-valued vector $\vec{x}$ 
to a single binary value $f(\vec{x})$, employing a linear activation function. The addition of hidden layers and 
non-linear activation functions led to the MLP, which could produce an arbitrary number of continuous output values. 
...
artificial neurons, as they have no activation function. Their only purpose is to forward the NN inputs to the 
...
is also valid for a NN with several outputs; the result applies to each output node individually. 
...
These problems are solved in practice by employing various techniques, illustrating that the universal approximation theorem
...
\subsubsection{Cost functions} \label{costFunctions}
...
stated that this thesis deals with neural network potentials (NNP). Then, in \autoref{sec:machineLearning}
...
(\autoref{sec:forceCutoff}). Thus, the NNP can be applied to MD simulations in exactly the same way 
...
All Si atoms have identical atomic NNs and symmetry function sets, the former represented by the function $F_{\mathrm{Si}}$. 
...
\begin{equation}
 ...
 \label{G1}
\end{equation}
We immediately realize that this simple function meets the symmetry requirement stated above: The value 
...
into account that the angular contribution depends on the atomic separations. The angular arrangement can be 
...
cosine function. For $\lambda = +1$ the maximum of the cosine terms is at $\theta_{jik} = 0^\circ$, 
while for $\lambda=-1$ it is located at $\theta_{jik} = 180^\circ$. The presence of the cutoff function $r_{jk}$
ensures that only triplets where all three interatomic distances are within the cutoff radius are included. 
...
In a MD simulation we only work with relative or interatomic coordinates and potentials. 
...
apply to each term in the sum separately. Further, the symmetry is also present for each atom in a triplet,
\begin{equation}
...
# thermalise before sampling rdf
run				10000
...
system during time stepping. The above \texttt{fix} 
...
which defines a computation that is performed on a group of atoms, in this case \texttt{all} atoms. 
...
LAMMPS allows several run commands to be issued
...
\begin{figure}
 ...
 \includegraphics[width = 0.8\linewidth]{Figures/Implementation/lammpsClasses.pdf}
  \caption{Class hierarchy of LAMMPS. The inheritance is from left to right. 
	   ...
	   of sub-classes (green and yellow) are displayed, where we recognize many of the input script
	   commands.}
...
If there are more than one element, a separate NN file is read for each one. 
...
\end{aligned}
\end{equation}
The NN is differentiated by employing backpropagation according to the procedure outlined in \autoref{sec:symmAndForces},
\begin{minted}{c++}
...
for (int s=0; s < m_numberOfSymmFunc; s++) {
  if ( m_parameters[s].size() == 3 ) {
    ...
    // calculate derivative of G2 for all pairs
...
\texttt{fj3} and \texttt{fk3} to atom $i$, while giving the negative of these forces to atom $j$ and $k$ separately.
...
\end{lstlisting}
will test if the installation was successful. 
...
The Python API is at present the most
...
\autoref{sec:extendingLammps} and \autoref{sec:applyingNNP}. 
...
The equations for evaluating a NN were \eqref{forwardPropMatrix},
\begin{subequations}
 \begin{align}
...
\begin{minted}{Python}
 initOperation = tf.global_variables_initializer()
\end{minted}
Constants are initialized when we call \texttt{tf.constant}, and their value can never change. 
By contrast, variables need to be initialized using a separate operation within a session. 
...
If each range is divided into $N$ equal segments, the total number of distinct configurations is $N^3$. 
...
that will constitute the initial data set. It has also been shown \cite{Raff05} that the final PES obtained 
...
Our strategy is to select a few atoms, usually 1-10, and compute $\tau$ separately for each of these atoms. When 
...
when the parameters of the employed potential is initialized with the input script command \texttt{pair\_coeff}. 
...
\vspace{-1ex}
  \caption{Distribution of forces for one Si atom in a quartz $\textrm{SiO}_2$ crystal with 
	   and without the use of the sampling algorithm \eqref{samplingAlgorithmModified}. 
...
of quartz $\textrm{SiO}_2$, with and without the use of the sampling algorithm \eqref{samplingAlgorithmModified}. 
...
the mean separation distance $d_\textrm{mean} = \langle\vec{d}\rangle$. 
...
In this section we discuss various techniques for building a suitable symmetry function set. 
...
Thus we occasionally refer to a single symmetry function as an \textit{input variable}. The symmetry function definitions 
...
functions are discussed below). Further, the distribution of interatomic distances in the data set may be investigated to acquire a 
...
with the largest $\eta$ value should correspond to the shortest interatomic distance present in the data set 
and vice versa. 
...
should increase exponentially to ensure that the symmetry functions are sufficiently dissimilar, i.e.\ to \textit{avoid
...
One could immediately conclude that we
...
This can be avoided by analysing the distribution of values of each symmetry function on the data set. If the difference 
...
configuration. Additionally, we can have an arbitrary number of hidden layers and nodes. 
...
This expression is almost equivalent to the cost function \eqref{quadraticCost} used in the optimization of the NN.
...
According to Larochelle et al.\ \cite{Larochelle09}, having the same number of nodes in each hidden layer gives the best results. 
...
in the potential and in the forces, and has also been employed by Behler in a modified form \cite{Behler15},
\begin{equation}
...
Lennard-Jones potential \eqref{shiftedLJ}. The quality of the fit will be analysed, but the NNP will not be applied 
...
The training set is in this case uniformly distributed, thus the error is larger for small interatomic distances where 
...
of interatomic distance $r_{ij}$ and triplet angle $\theta_{jik}$ respectively. 
...
because it is more computationally efficient compared to the hyperbolic tangent, especially 
its derivative,
\begin{equation}
 f^\prime(x) = f(x) [1 - f(x)]
\end{equation}
...
if testRMSE / trainRMSE > 10:
    print 'Overfitting is occurring, training ends'
    break
\end{minted}
...
We use the same hyperparameters as in \autoref{tab:hyperParamsSiInitial} (with various architectures), 
...
The results are listed in \autoref{tab:gridSearch}. We immediately observe that all RMSEs lie in a range of about 1.5-2.5 meV, except for the two cases where $n_l = 4$. 
...
are equally probable, and $g(r)$ is unity for all distances. Any deviations from unity signifies an ordered structure.
...
To measure $g(r)$, we prepare a system consisting of 1000 Si atoms on a diamond cubic lattice with a lattice constant 
...
\end{equation}
These quantities can be obtained by employing Hooke's law. For an anisotropic material like Si, Hooke's law is written as a tensor 
equation
\begin{equation}
...
for the construction of an interatomic potential for solid-state Si, and the radial distribution function,
...
of 0.896 meV. The absolute error was plotted as a function of interatomic distance, demonstrating that the error 
...
\begin{enumerate}
 ...
 \item \textit{There should be ways to systematically detect situations where the potential is not sufficiently accurate, 
 ...
 \item \textit{The potential should be high-dimensional, i.e.\ depend on all degrees of freedom of the system.}
 ...
 \item \textit{The potential should be fast to evaluate.}
       ...
       The NNP is several orders of magnitude faster to evaluate than e.g.\ DFT or Hartree-Fock potentials, 
...
for conventional \textit{ab inito} MD. This property, in addition to the unbiased and transferable nature of the 
...
\begin{equation}
 ...
 \frac{x_{ij}}{r_{ij}}
 \label{cutOffFunctionDerivative2}
\end{equation}
and 
\begin{equation}
 \frac{\partial f_c(r_{ik})}{\partial x_{ik}} = M(r_{ik})
 \frac{x_{ik}}{r_{ik}}
 \label{cutOffFunctionDerivative3}
\end{equation}
while
\begin{equation}
...
\begin{equation}
 ...
 \frac{x_{jk}}{r_{jk}}
 \label{cutOffFunctionDerivative4}
\end{equation}
and 
\begin{equation}
 \frac{\partial f_c(r_{jk})}{\partial x_{ik}} = M(r_{jk})
 \frac{x_{jk}}{r_{jk}}
 \label{cutOffFunctionDerivative5}
\end{equation}
