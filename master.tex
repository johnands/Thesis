...
assuming the the activation function is a non-constant, bounded and monotonically-increasing continous function. 
The only requirement on the NN is that it must be feed-forward, thus recurrent NNs are out of the picture. 
...
(explained in sec (REF TO SEC)), we also add a \textit{bias} to this model, resulting in 
\begin{equation}
...
$w_{ij}^l$ is the weight connecting node $i$ in layer $l-1$ with node $j$ in layer $l$. All weights and biases are real-valued numbers.
...
A desired property of activation functions is to converge asymptotically
