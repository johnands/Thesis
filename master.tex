...
\begin{equation}
 y^\prime = f^\prime(u)
\end{equation}
where $f$ is the activation function. We see from \autoref{fig:sigmoidActivationFunctions} that the sigmoid and hyperbolic
tangent have very small gradients for all inputs except those in the non-linear region around zero.
Large positive or negative inputs therefore results in vanishing gradients and tiny updates, slowing down learning. 
This is yet another reason for shifting the mean of the input training data to zero, and it also has 
directs the initialization of the weights. 
