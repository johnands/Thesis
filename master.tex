...
The number of inputs and outputs thus defines
the dimensions of the function we want to fit: 
...
\begin{equation}
 y^2_i = w^2_{i1}y^1_1 + w^2_{i2}y^1_2 + w^2_{i3}y^1_3 + b_i = 
 \sum_{j=1}^3 w_{ij} y_j^1 + b_i
\end{equation} 
...
The training set $X$ and reference set $Y$ are represented as matrices, where the $i$-th row of $X$ and the $i$-th row of $Y$
...
Different kinds of gradient descent-based methods are widely used, but also higher order methods like
...
\begin{equation}
 \vec{\theta}^{\mathrm{new}} = \vec{\theta}^{\mathrm{old}} - \gamma \nabla_{\vec{\theta_\mathrm{old}}} \Gamma
...
algorithm of choice, as it has the optimal trade-off between speed and accuracy. NORMAL BATCH SIZES
...
\end{equation}
but to implement these we need the partial derivatives of
the cost function w.r.t. to all weights and biases to determine how we should adjust each weight.
