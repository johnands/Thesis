...
by evaluating the trained NN on a set of distance values $R_{ij} \in [a,b]$, and compare with the target function,
\begin{minted}{Python}
# make vector of N distance values r
N = 2000
Rij = np.linspace(self.a, self.b, N)

# evaluate trained network on this interval and find absolute error
energiesNN = sess.run(prediction, feed_dict={x: Rij.reshape([N,1])})
energiesLJ = self.LennardJones(Rij)
...
\begin{figure}
\centering
  \includegraphics[width = 0.8\linewidth]{Figures/Implementation/LJError.pdf}
  \caption{Absolute error of a NN trained to reproduce the truncated and shifted Lennard-Jones potential \eqref{shiftedLJ}. 
           The plot can be viewed as a visualization of the error distribution in configuration space, of which the latter 
           is defined as $R_{ij} \in [a,b]$. The employed hyperparameters are listed in \autoref{tab:hyperParamsLJ}.} 
  \label{fig:LJError}
\end{figure}
